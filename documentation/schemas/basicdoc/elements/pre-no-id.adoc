...
// TODO: salvage prose for this non-terminal. The anchor `[#pre-no-id]`
// above is the canonical stable ID that grammar `## @see <<pre-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-pre-no-id[basicdoc.html#define-pre-no-id]
