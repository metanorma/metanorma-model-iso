...
// TODO: salvage prose for this non-terminal. The anchor `[#image-no-id]`
// above is the canonical stable ID that grammar `## @see <<image-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-image-no-id[basicdoc.html#define-image-no-id]
