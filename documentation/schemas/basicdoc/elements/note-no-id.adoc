...
// TODO: salvage prose for this non-terminal. The anchor `[#note-no-id]`
// above is the canonical stable ID that grammar `## @see <<note-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-note-no-id[basicdoc.html#define-note-no-id]
