...
// TODO: salvage prose for this non-terminal. The anchor `[#thead-no-id]`
// above is the canonical stable ID that grammar `## @see <<thead-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-thead-no-id[basicdoc.html#define-thead-no-id]
