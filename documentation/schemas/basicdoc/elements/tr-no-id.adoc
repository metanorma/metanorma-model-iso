...
// TODO: salvage prose for this non-terminal. The anchor `[#tr-no-id]`
// above is the canonical stable ID that grammar `## @see <<tr-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-tr-no-id[basicdoc.html#define-tr-no-id]
