...
// TODO: salvage prose for this non-terminal. The anchor `[#example-no-id]`
// above is the canonical stable ID that grammar `## @see <<example-no-id>>`
...
See the corresponding SPA panel: link:../../../../basicdoc.html#define-example-no-id[basicdoc.html#define-example-no-id]
