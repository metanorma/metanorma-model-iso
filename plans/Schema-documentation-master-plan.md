...
2. **~~Tactical workaround~~** in [~~PR #127~~](https://github.com/metanorma/metanorma-model-iso/pull/127): strip the outer `grammar { ... }` block from each `grammars/*.rnc`. **Reverted in [PR #128](https://github.com/metanorma/metanorma-model-iso/pull/128) — wrong diagnosis.** After CI ran with the strip, all seven layers including the previously-non-zero `relaton-iso` dropped to 0/0/0/0. The CI log surfaced the actual error: lutaml/rng's parser accepts three top-level forms — a `grammar { ... }` block, OR one or more `include` directives, OR a single `GRAMMAR` token — and **bare top-level `define`s without a wrapper are not valid RNC**. Stripping the wrapper made the files un-parseable; `relaton-iso-compile.rnc`'s include chain then cascaded.

**Corrected diagnosis.** The lutaml/rng parser **does** accept the `grammar { ... }` block (no parse error pre-strip). The bug is downstream: lutaml-xsd's RNC-to-XSD converter sees the `inner_grammar:GRAMMAR_BLOCK` AST node but doesn't iterate into its body to extract the inner `define` / `element` / `attribute` / `group` nodes for the LXR. `relaton-iso-compile.rnc`'s 2-elements-1-group outlier confirms this — it uses the top-level-includes form, no grammar block, and the LXR builder DOES extract the `start =` declaration along that code path. The grammar-block branch's extraction is missing.

lutaml/rng#19 has been updated to reframe the bug against the converter / LXR builder rather than the parser. No tactical workaround on the metanorma side is feasible: the only semantics-preserving transformation that keeps the files parseable also keeps them in the wrapped form that triggers the bug. The seven SPAs stay at their degraded baseline (six layers at 0; `relaton-iso` at 2/1) until the upstream fix lands.
